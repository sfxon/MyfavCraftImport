...
class CmsPageImportService
{
    ...
    public function process(Context $context, mixed $importData, ?array $fieldDataIndex): mixed
    {
        ...
        if(!isset($pluginConfig['cmsPageId']) || $pluginConfig['cmsPageId'] === null) {
            throw new \Exception('Default cmsPageId is not configured in plugin configuration.');
        }

        return $pluginConfig['cmsPageId'];
    }
}